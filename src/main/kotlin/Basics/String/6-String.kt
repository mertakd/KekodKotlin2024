...
fun main() {
     ...
     *
     * name stack de, Tolga heap de yer alacak sonra değeri Öztürk yapınca heap de Öztürk değeri de var olacak.
     * daha sonra garbage collector heap i düzenli aralıklar la gezer bakar eğer işaret edilmeyen yani referans edilmeyen bir value kaldıysa bunu siler.
       Böyle bir durumda yani garbage collector çalıştığı durumda  Tolga memory den gider, öztürk kalır.
       Ama Garbage collector çalışana kadar ikisi beraber memory de yer alacaklar.

     * String ler var veya val olsalar da immutable dır. yukarıda da anlatıldığı için ilk değer değiştirilse bile  hala heap de kaldığı için immutable dır. 2:54:00 Ders5: Kotlin | Basic Types Part3
     */

    var name = "Tolga"
    name = "Öztürk"

    println(name.uppercase()) // "ÖZTÜRK"
    println(name) // Öztürk
    ...
    //  val numbersValue2: String = (4 + 2 + 6) + "value"     hata verir
    val numbersValue3: String = "value".plus(4 + 2 + 6) //  çalışır, value12
    //val numbersValue4: String = (4 + 2 + 6).plus("value")// çalışmaz
