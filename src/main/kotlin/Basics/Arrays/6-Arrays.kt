...
fun main() {
     ...
     * bir array oluşturur.
     *
     * Array(){} yaparak işlem yapmak: burada bir işlem yaptıktan sonra değerlerin atanmasını istiyor olabilirz.
     */
    ...
    val studentNotes2 = Array(5) { 0 } // 5 elemanlı bir dizi oluşturulur ve başlangıçta tüm elemanları sıfır olarak ayarlanır

    // Dizinin her bir elemanına rastgele notlar atanabilir
    for (i in studentNotes2.indices) {
        studentNotes2[i] = (0..100).random() // Her elemana 0 ile 100 arasında rastgele bir not atanır
    }

    // Dizinin her bir elemanı ekrana yazdırılır
    println("Öğrenci notları:")
    for (i in studentNotes2.indices) {
        println("Öğrenci ${i+1}: ${studentNotes2[i]}")
    }
